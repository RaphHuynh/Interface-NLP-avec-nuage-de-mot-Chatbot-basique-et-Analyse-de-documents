...
def appliquer_stemming_corpus(corpus: List[List[str]], stemming: str) -> List[List[str]]:
    """
    Appliquer le stemming à un corpus en fonction de l'option choisie.
    
    Args:
        corpus (List[List[str]]): Le corpus à traiter.
        stemming (str): L'option pour choisir le stemming.
        
    Returns:
        List[List[str]]: La liste des documents après application du stemming.
    
    """
    return [appliquer_stemming(phrase, stemming) for phrase in corpus]